...
outpath <- "K:/DataServices/Projects/Current_Projects/Projections/Projections_2023/Data/04_Allocation_Results/Revised_Allocation_Sept2025/Tabular/"
...
# Create a list of years to pull the data from.
year_list <- c(2019)

for (i in year_list) {
  # Print statement for for-loop progress
  print(paste0("Loading Urbansim outputs for ", i, "."))
  
  # Load each year of household and employment data. Put into tmp dataframes to be 
  tmp_hhds <- read.csv(paste0(outpath, "urbansim_complete_microsim_population_website_revised_",i,".csv"))
  
  tmp_emp.swm <- read.csv(paste0(outpath, "swm_urbansim_employment_",i,".csv"))
  
  tmp_emp.mapcm <- read.csv(paste0(outpath, "mapcm_urbansim_employment_",i,".csv"))
  
  # Add tmp dataframes to the full household or employment dataframe.
  hhds <- bind_rows(hhds, tmp_hhds)
  
  emp.swm <- bind_rows(emp.swm, tmp_emp.swm)
  
  emp.mapcm <- bind_rows(emp.mapcm, tmp_emp.mapcm)
  
  # Remove tmp dataframes
  rm(tmp_hhds, tmp_emp.swm, tmp_emp.mapcm)
