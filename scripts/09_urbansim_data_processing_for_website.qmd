...
```{r}
# 5.3.1 Total Projected Households State
# State


test <- hh.bc.1 |> filter(if_all(everything(), ~ !is.na(.x)))
...
# 5.3.2 Projected Households by Household Type
# State
hh.bc.3 <- hhds.type |> 
  mutate(state = 1) |> 
  filter(!`Household Type` == "Total Households") |> 
...
graph <-
  mv.bar.chart_(
    ...
    x_var = "year",
    y_var = "hh",
    group_var = "Household Type",
    x_label = "Year",
    y_label = "Households",
    group_label = "Household Type",
    custom_title = "Projected Households",
    geo = "Massachusetts"
  )
...
# 5.3.3 Projected Households by Household Size
# State
# RPA

# 5.3.4 Projected Households by Householder Age
# State
# RPA
```
...
```{r}
# 5.4.1 Total Projected Employment
# State
# State
emp.bc.1 <- emp |> 
  mutate(state = 1) |> 
...
# 5.4.2 Projected Employment by Supersectors
# State
# RPA
...
graph <-
  sv.bar.chart_(
    ...
    x_label = "Year",
    y_label = "Population",
    custom_title = "Projected Population of Children",
    geo = "Massachusetts"
  )
...
ggsave(
  filename = paste0("pop_children.jpeg"),
  plot = get_last_plot(),
  path = output_path,
  width = 10,
  dpi = 320,
...
for (r in RPA) {
  ...
  # Save graph to disk.
  ggsave(
    filename = paste0("pop_children.",r,".jpeg"),
    plot = get_last_plot(),
    path = output_path,
    width = 10,
    dpi = 320,
...
# 5.6 Change Demographic Components of Projected Households (Stacked Bar Charts)
...
# 5.7 Change Components of Projected Employment (Stacked Bar Charts)
...
# 5.8 Change Projected Population (Stacked Bar Charts)
