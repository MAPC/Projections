...
dir <- 'Microsim_outputs/TDM23.3.0'
...
phh <- fread(paste0(r1,'PUMS/ss13hma.csv'))
pper <- fread(paste0(r1,'PUMS/ss13pma.csv'))
pums <- left_join(pper, phh, by='SERIALNO')

pums <- pums[,c(2,3,9:215,296,298,301,302,308:347,350:419), with=F]
pums[,serialno:=as.character(SERIALNO)]
pums[,person_num:=SPORDER]
setkey(pums,serialno,person_num)

for (yr in c(2019,2024,2049)){
dt <- fread(paste0(root,dir,'/urbansim_complete_population_',
      ctps_run,
      '_',
      yr,
      '.csv'))
dt <- dt[,c(1:30,49:53,56,57),with=F]
dt[,serialno:=as.character(serialno)]

dexp <- left_join(dt,pums,by=c('serialno','person_num')) |> 
  setDT()

fwrite(dexp,paste0(root,dir,'/urbansim_population_complete_PUMS_records_',
      ctps_run,
      '_',
      yr,
      '.csv'))
