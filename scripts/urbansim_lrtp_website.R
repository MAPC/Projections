...
# !!NOTE!! The zoning constraint for ROW blocks was not effective in preventing allocation
# of households and jobs to blocks in the ROW in mostly Boston/Cambridge
...
mb <- fread(
  'S:/Network Shares/K Drive/DataServices/Projects/Current_Projects/Projections/Projections_2023/Data/03_UrbanSim/MassBuilds/all_devprojects_8_19_2025.csv'
) |>
  select(
    block_id,
    identification,
...
dt <- left_join(d10, d50, by = c('block_id', 'muni_id')) |>
  ...
  mutate(
    ...
    mb_units = case_when(is.na(mb_units) ~ 0, .default = mb_units),
...
gone.over <- dt[units2010 <= unit_capacity &
                  units2050 > unit_capacity]
at.cap2050 <- dt[units2010 <= unit_capacity &
                   units2050 == unit_capacity]
under2050 <- dt[units2050 < unit_capacity]

gone.over[overbuild > mb_units]

cnty <- c('001','003','005','007','009','011','013','015','017','019','021','023','025','027')
...
dec <- get_decennial(
  ...
  select(GEOID, value) |>
  rename(censusHH2010 = value, bl10_id = GEOID)

sf <- mapcdatakeys::block_sf(2010) |>
  left_join(dt, by = c('bl10_id', 'muni_id')) |>
  select(-block_id) |> 
  left_join(dec, by='bl10_id')

d2 <- setDT(sf)

st_write(sf,
         paste0(root, 'lrtp_housing_units_and_zoning_capacity_block.shp'))
fwrite(d2,
       paste0(root, 'lrtp_housing_units_and_zoning_capacity_block.csv'))

sw <- fread(
  paste0(
    r2,
    'Data/03_UrbanSim/Constraints/block_constraint_swm_zoning_inter.csv'
  )
) |>
  mutate(DU = maxDUA_rev * sub_acres, bl10_id = as.character(bl10_id)) |>
  group_by(bl10_id) |>
  summarise(maxDU = sum(DU), acres = sum(sub_acres)) |>
  right_join(d2, by = 'bl10_id')
...
er <- fread(paste0(
  r2,
  'Data/03_UrbanSim/Constraints/blocks2010_constraints_erased.csv'
)) |>
  mutate(bl10_id = as.character(GEOID)) |>
  select(bl10_id, area_acres) |>
  right_join(sw, by = 'bl10_id') |>
  mutate(acres = case_when(is.na(acres) &
                             !is.na(area_acres) ~ area_acres, .default = acres))

df <- exp |> select(GEOID, max_dua) |>
  right_join(er, by = c('GEOID' = 'bl10_id')) |>
  mutate(maxDU = case_when(is.na(maxDU) &
                             !is.na(max_dua) ~ max_dua * acres, .default = maxDU)) |>
  setDT()
