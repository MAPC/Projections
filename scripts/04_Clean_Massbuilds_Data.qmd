---
title: "Clean Massbuilds Data for UrbanSim"
format: html
editor: visual
---
...
# Massbuilds vintage year/month/day
vintage = '20260324'
# domain = 'Statewide'
domain = 'MAPC'
...
if (grepl('VM',Sys.info()[['nodename']])){
  root <- '//data-sync/Public/DataServices'
} else {
  root <- 'S:/Network Shares/K Drive/DataServices'
}

# Root Directory
data_path <- file.path(root,'Projects/Current_Projects/Projections/Projections_2028/Data/02_UrbanSim/Massbuilds')


# Use ArcGIS python.exe path
# arc.check_product()
...
# 3 - Reverse geocode to get full address field

```{r}

res <- fread(paste0(data_path,'/Massbuilds_Inputs_',vintage,'/',domain,'/',vintage,'_residential.csv'))
com <- fread(paste0(data_path,'/Massbuilds_Inputs_',vintage,'/',domain,'/',vintage,'_non_residential.csv'))

dt <- fread('Massbuilds_to_add_to_UrbanCanvas_20260325.csv')
...
for (i in 1:nrow(gc)){
  dt <- as.data.table(gc$address_components[[i]])
  dt$types <- sapply(dt$types, paste, collapse = ", ")
  num <- dt[types=='street_number',long_name]
  strt <- dt[types=='route',long_name]
  muni <-  dt[types=='locality, political',long_name]
  zip <-  dt[types=='postal_code',long_name]
  addr <- paste(num, strt, muni, 'MA', zip)
  gc[i, full_address:=addr]
  rm(dt)
}
setnames(gc,'full_address...8','full_address2')
setnames(gc,'full_address...17','full_address3')
...
df <- dt[address!='']
df[is.na(zip),zip:=0]
df[,full_address:=paste(address,city,state,sprintf("%05d",zip))]

ms <- all_muni_data_keys |> select(muni_name,rpa_acr)
cln <- rbind(df,gcf,fill=TRUE) |> 
  left_join(ms, by=c('city'='muni_name'))
fwrite(cln,'Massbuilds_to_add_to_Urbancanvas_20260325_clean.csv')
```
